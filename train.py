...
def eval(dataloader, faster_rcnn, test_num=10000):
    ...
    for ii, (imgs, sizes, gt_bboxes_, gt_labels_, gt_difficults_) in enumerate(tqdm(dataloader, total=len(dataloader))):
...
def eval_asr(dataloader, faster_rcnn, atk_model, test_num=10000):
    ...
    if opt.attack_type == 'g':
        ...
        asr = get_ASR_m(gt_bboxes, atk_pred_bboxes, atk_pred_labels, atk_pred_scores, pred_bboxes, pred_labels,
                        pred_scores, opt.target_class)
    return asr
...
def train(**kwargs):
    ...
    for epoch in range(opt.checkpoint_epoch, opt.epoch):
        ...
        for ii, (img, bbox_, label_, scale) in tqdm(enumerate(dataloader), total=len(dataloader)):
            ...
            if (ii + 1) % opt.plot_every == 0:
                try:
                    ...
                    pred_img = visdom_bbox(opt.dataset, ori_img_, at.tonumpy(_bboxes[0]),
                    ...
                    atk_pred_img = visdom_bbox(opt.dataset, atk_ori_img_, at.tonumpy(_bboxes[0]),
                                               at.tonumpy(_labels[0]).reshape(-1), at.tonumpy(_scores[0]))
        ...
        best_path2 = atk_model.save(attack_type=opt.attack_type, best_asr=filename)
