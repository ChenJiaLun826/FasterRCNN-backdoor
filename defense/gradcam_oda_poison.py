...
def main():
    # ====== 你可以改这里 ======
    model_path = "checkpoints/ODA_fasterrcnn_04141613_0_0.504398510956926_0.2084460517785102_0.9662162162162162"
    atk_model_path = "checkpoints/ODA_autoencoder_04141613_0_0.504398510956926_0.2084460517785102_0.9662162162162162"
    target_class = 0   # ODA 看 background class
    epsilon = 0.06
    save_dir = "defense_results/gradcam_oda_poison"
    num_samples = 10
    # =========================
    ...
    for sample_index in range(num_samples):
        ...
        print("sample:", sample_index, "clean target_score:", float(target_score.detach().cpu()))

        cam_clean = gradcam.generate(target_score)

        img_rgb = to_rgb_uint8_from_chw(img)
        overlay_clean = overlay_cam_on_image(img_rgb, cam_clean)

        clean_path = os.path.join(save_dir, f"sample_{sample_index:03d}_clean.jpg")
        clean_cam_path = os.path.join(save_dir, f"sample_{sample_index:03d}_clean_gradcam_bg.jpg")

        cv2.imwrite(clean_path, cv2.cvtColor(img_rgb, cv2.COLOR_RGB2BGR))
        cv2.imwrite(clean_cam_path, cv2.cvtColor(overlay_clean, cv2.COLOR_RGB2BGR))

        # ===== poisoned image =====
        trigger = atk_model(img_tensor)
        resized_trigger = resize_image(trigger, img_tensor[0][0].shape)
        atk_imgs = clip_image(img_tensor + resized_trigger * opt.epsilon)

        faster_rcnn.zero_grad()
        roi_cls_locs_atk, roi_scores_atk, rois_atk, roi_indices_atk = faster_rcnn(atk_imgs, scale=scale)
        target_scores_atk = roi_scores_atk[:, target_class]
        target_score_atk = target_scores_atk.max()

        print("sample:", sample_index, "poison target_score:", float(target_score_atk.detach().cpu()))

        cam_poison = gradcam.generate(target_score_atk)

        atk_img_rgb = to_rgb_uint8_from_chw(atk_imgs[0].detach().cpu().numpy())
        overlay_poison = overlay_cam_on_image(atk_img_rgb, cam_poison)

        poison_path = os.path.join(save_dir, f"sample_{sample_index:03d}_poisoned.jpg")
        poison_cam_path = os.path.join(save_dir, f"sample_{sample_index:03d}_poisoned_gradcam_bg.jpg")
        cv2.imwrite(poison_path, cv2.cvtColor(atk_img_rgb, cv2.COLOR_RGB2BGR))
        cv2.imwrite(poison_cam_path, cv2.cvtColor(overlay_poison, cv2.COLOR_RGB2BGR))

        print("保存完成：")
        print(clean_path)
        print(clean_cam_path)
        print(poison_path)
        print(poison_cam_path)
